...
export const Scratchpad: React.FC = () => {
  ...
  const allTags = useMemo(() => {
    const flatList = recents.flatMap((note) => note.tags || []);
  ...
  if (isLoading) {
    return (
      <div className="relative h-full w-full flex-1 flex-col">Loading...</div>
    );
  } else {
    return (
            ...
            {recents.map((item) => (
              ...
              >
                ...
                <div
                  ...
                  style={{ gap: '24px' }}
                >
                  <span className="recent-item-preview flex-1">
                    {item.text}
                  </span>
                  <span
